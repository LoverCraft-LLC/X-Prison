...
import com.loverfella.commons.util.PlayerUtil;
    ...
    private void subscribeToPlayerInteractEvent() {
        Events.subscribe(PlayerInteractEvent.class)
                ...
                .handler(e -> {
                    ...
                    int pickaxeSlot = InventoryUtils.getInventorySlot(p, pickAxe);
                    this.plugin.getCore().debug("Pickaxe slot is: " + pickaxeSlot, this.plugin);

                    if (PlayerUtil.isBedrockPlayer(p)) {
                        if (p.isSneaking()) new EnchantGUI(this.plugin, p, pickAxe, pickaxeSlot).open();
                    } else {
                        new EnchantGUI(this.plugin, p, pickAxe, pickaxeSlot).open();
                    }
                }).bindWith(this.plugin.getCore());
    }
